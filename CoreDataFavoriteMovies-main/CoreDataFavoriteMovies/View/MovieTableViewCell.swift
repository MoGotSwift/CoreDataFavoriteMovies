...
class MovieTableViewCell: UITableViewCell {
    ...
    func update(with movie: APIMovie, onFavorite: (() -> Void)?) {
        ...
        if MovieController.shared.fetchFavoriteMovies().contains(where: { favorite in
    ...
    func update(with movie: Movie, onFavorite: (() -> Void)?) {
        self.movie = movie
        self.onFavorite = onFavorite
        posterImageView.kf.setImage(with: URL(string: movie.posterURL!), placeholder: placeholder)
        titleLabel.text = movie.title
        yearLabel.text = movie.year
        setFavorite()
    }
