...
private extension MovieSearchViewController {
    ...
    func toggleFavorite(_ movie: APIMovie) {
        let favoriteMovies = movieController.fetchFavoriteMovies()
        if favoriteMovies.contains(where: { $0.imdbID == movie.imdbID }) {
            if let movieToDelete = favoriteMovies.first(where: { $0.imdbID == movie.imdbID }) {
                movieController.unfavoriteMovie(movieToDelete)
            }
        } else {
